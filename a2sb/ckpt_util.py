...
def download_adm_image512_uncond_ckpt(ckpt_dir="data/"):
    ...
    opt = create_argparser_image512()

    adm_ckpt = os.path.join(ckpt_dir, opt.adm_ckpt)
    if not os.path.exists(adm_ckpt):
        print("Downloading ADM checkpoint to {} ...".format(adm_ckpt))
        download(ADM_IMG512_UNCOND_CKPT, adm_ckpt)
    ckpt_state_dict = torch.load(adm_ckpt, map_location="cpu")
...
def download_ckpt(ckpt_dir="data/"):
    ...
    download_adm_image512_uncond_ckpt(ckpt_dir=ckpt_dir)
    download_adm_image512_cond_ckpt(ckpt_dir=ckpt_dir)

def build_ckpt_option(opt, log, ckpt_path, net="sbae"):
    ...
    ckpt_opt.load = ckpt_path / f"{opt.load_itr:0>7}.pt" if net == "sbae" else ckpt_path / f"{opt.ldm_load_itr:0>7}.pt"
    return ckpt_opt
