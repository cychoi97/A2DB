...
class Runner(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        self.diffusion = create_gaussian_diffusion(steps=opt.interval,
                                                   noise_schedule=opt.schedule_name)
        log.info(f"[Diffusion] Built latent diffusion: steps={opt.interval}!")

        noise_levels = torch.linspace(opt.t0, opt.T, opt.interval, device=opt.device) * opt.interval
        
        if opt.image_size == 256:
            self.net = MLPNet(num_channels=512, num_hidden_channels=1024, num_layers=10, skip_layers=list(range(1, 10)), use_fp16=False)
            self.sbae = Image256Net(log, noise_levels=noise_levels, image_size=opt.image_size, in_channels=sbae_opt.in_channels,
                                    use_fp16=sbae_opt.use_fp16, cond=sbae_opt.cond_x1)
        elif opt.image_size == 512:
            self.net = MLPNet(num_channels=1024, num_hidden_channels=2048, num_layers=20, skip_layers=list(range(1, 20)), use_fp16=False)
            self.sbae = Image512Net(log, noise_levels=noise_levels, image_size=opt.image_size, in_channels=sbae_opt.in_channels,
                                    use_fp16=sbae_opt.use_fp16, cond=sbae_opt.cond_x1)

        self.ema = ExponentialMovingAverage(self.net.parameters(), decay=opt.ema)
        self.sbae_ema = ExponentialMovingAverage(self.sbae.parameters(), decay=sbae_opt.ema)
        ...
        self.sbae.load_state_dict(sbae_checkpoint['net'])
    ...
    def sample_batch(self, opt, loader):
        ...
        with self.sbae_ema.average_parameters():
            self.sbae.eval()
            z_sem = self.sbae.semantic_enc(x0).detach().clone().cpu()
        return z_sem
    ...
    def train(self, opt, train_dataset):
        ...
        for it in range(opt.start_itr, opt.num_itr + 1):
            ...
            for _ in range(n_inner_loop):
                # ===== sample style embed =====
                z_sem = self.sample_batch(opt, train_loader)
