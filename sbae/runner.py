...
class Runner(object):
    ...
    def train(self, opt, train_dataset, val_dataset):
        ...
        for it in range(opt.start_itr, opt.num_itr + 1):
            ...
            for _ in range(n_inner_loop):
                ...
                pred, style_emb = net(x0, xt, step, cond=cond)
                assert x0.shape == xt.shape == label.shape == pred.shape

                loss = F.mse_loss(pred, label) + 0.00001 * torch.mean(style_emb ** 2)
                loss.backward()
    ...
    def ddpm_sampling(self, opt, x0, x1, cond=None, generated_style=None, clip_denoise=False, nfe=None, log_count=10, verbose=True):
        ...
        with self.ema.average_parameters():
            ...
            def pred_x0_fn(xt, step):
                ...
                out, _ = self.net(x0, xt, step, cond=cond, generated_style=generated_style)
    ...
    def evaluation(self, opt, it, val_loader, ckpt_path):
        ...
        # if opt.cond_x1:
        #     img_cond    = all_cat_cpu(opt, log, cond)
        ...
        # if opt.cond_x1:
        #     tu.save_image(tu.make_grid((img_cond+1)/2, nrow=10), os.path.join(image_path, f'{it:07}_cond_x1.png'), value_range=(0, 1))
